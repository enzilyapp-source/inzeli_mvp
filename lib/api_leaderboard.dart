...
class ApiLeaderboard {
  static Future<List<Map<String, dynamic>>> globalTop(
      {String? token, int limit = 30, String? gameId}) async {
  ...
  static Future<List<Map<String, dynamic>>> sponsorGameTop({
    ...
    final uri = Uri.parse(
        '$apiBase/leaderboard/sponsor?sponsorCode=$sponsorCode&gameId=$gameId&limit=$limit');
    final res = await http.get(uri, headers: {
      if (token != null && token.isNotEmpty) 'Authorization': 'Bearer $token',
    });
